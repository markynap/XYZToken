...
contract FeeReceiver is Ownable {
    ...
    // Fund Fees
    uint public interestFundFee = 70;
    ...
    // events
    event UpdatedFees(uint interestFee, uint burnFee, uint liquidityFee);
    ...
    function trigger() external {
        ...
        // fetch token balance
        uint tBal = token.balanceOf(address(this));
        if (tBal <= 100) return;

        // split up amounts
        uint lAmt = ( tBal * liquidityFee ) / feeDenom();
        uint iAmt = ( tBal * interestFundFee ) / feeDenom();
        ...
        // swap tokens for BNB
        token.approve(address(router), iAmt);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(iAmt, 0, path, address(this), block.timestamp + 300);
    ...
    function updateFees(uint nInterestFee, uint nBurnFee, uint nLiquidityFee) external onlyOwner {
        interestFundFee = nInterestFee;
        burnFee = nBurnFee;
        liquidityFee = nLiquidityFee;
        emit UpdatedFees(nInterestFee, nBurnFee, nLiquidityFee);
    }
